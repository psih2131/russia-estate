    ...
    <main class="main">
        ...
        <section class="blog-sec">
                ...
                <div class="blog-sec__wrapper">
                    <h1 class="blog-sec__title">Блог</h1>
                    ...
                    <nav class="blog-sec__nav">
                        <ul class="blog-sec__nav-ul">
                            <li class="blog-sec__nav-li">
                                <a class="blog-sec__nav-link blog-sec__nav-link--activ">Все посты</a>
                            </li>
                        ...
                        <div class="pagination">
                            ...
                            <ul class="pagination__num-list">
                                <li class="pagination__nam-li " v-for="(item, index) in totalPages" :key="index" :class="{'pagination__nam-li--activ' : item == currentPage }">
                                    ...
                                    <NuxtLink :to="`/blog/?page=${item}`" class="page-numbers">
                                        {{ item }}
                                    </NuxtLink>
...
const { data: all_object, error, pending } = await useFetch(`${store.serverUrlDomainRequest}/wp-json/wp/v2/blog?page=${currentPage.value || 1}&per_page=${perPage.value}`, {
    onResponse({ response }) {
...
async function fetchClientData() {
  const res = await fetch(`${store.serverUrlDomainRequest}/wp-json/wp/v2/blog?page=${currentPage.value || 1}&per_page=${perPage.value}`)
...
//         { property: 'og:image', content:  'http://syberia.gearsdpz.beget.tech/wp-content/uploads/2025/07/87baa9efe5d849e4f8da67fe01f9e029.jpg' },

//         // Twitter Card (если используешь)
//         { name: 'twitter:card', content: 'summary_large_image' },
//         { name: 'twitter:title', content: 'test' },
//         { name: 'twitter:description', content: 'test' },
//         { name: 'twitter:image', content:  'http://syberia.gearsdpz.beget.tech/wp-content/uploads/2025/07/87baa9efe5d849e4f8da67fe01f9e029.jpg' },
